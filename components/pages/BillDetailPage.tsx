...
export default function BillDetailPage({
  ...
  const handleConfirmPaid = async (personId: string) => {
    try {
      ...
      await onUpdateBill(bill.id, { paidStatus: newPaid, pendingStatus: newPending });
    } catch (error) {
      ...
      <Card className="p-4 md:p-6">
        <div className="flex justify-between items-start mb-4 md:mb-6 gap-2">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-gray-500 text-xs md:text-sm">Paid by {paidByPerson?.name}</p>
          </div>
          <button
        ...
        <h3 className="font-bold text-gray-900 mb-4">
          {bill.splitType === 'equal' ? 'Split equally among:' : 'Split breakdown:'}
        </h3>
        <div className="space-y-3">
          {amounts.map((item) => {
            const person = allParticipants.find((p) => p.id === item.personId);
            const isPaid = bill.paidStatus?.[item.personId] || false;
            const canTogglePaid = item.personId !== bill.paidBy && authenticatedUser?.uid === item.personId;

            return (
              <div
                key={item.personId}
                className={`flex items-center justify-between p-3 border rounded-xl transition-all ${
                  isPaid ? 'border-green-200 bg-green-50' : 'border-gray-100'
                }`}
              >
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900">{person?.name || 'Unknown'}</p>
                ...
                {item.personId !== bill.paidBy && (
                  <div className="flex items-center gap-2">
                    {/* If already paid */}
                    {isPaid && (
                      <div className="text-xs font-semibold text-green-600 ml-2">Paid</div>
                    )}

                    {/* If pending (participant marked paid but awaiting payer confirmation) */}
                    {bill.pendingStatus?.[item.personId] && !isPaid && (
                      <div className="text-xs font-semibold text-yellow-600 ml-2">Pending</div>
                    )}

                    {/* Participant can mark themselves as paid (sets pending) */}
                    {authenticatedUser?.uid === item.personId && !isPaid && !bill.pendingStatus?.[item.personId] && (
                      <button
                        onClick={() => handleMarkPaid(item.personId)}
                        disabled={updating}
                    ...
                    {authenticatedUser?.uid === bill.paidBy && bill.pendingStatus?.[item.personId] && !isPaid && (
                      <button
                        onClick={() => handleConfirmPaid(item.personId)}
                        disabled={updating}
