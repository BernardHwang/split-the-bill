...
export default function DashboardPage({
  ...
  const calculateAmounts = (bill: Bill, userId: string) => {
    if (bill.splitType === 'equal') {
      ...
      const itemizedTotal = Object.values(bill.itemizedAmounts || {}).reduce((a, b) => a + b, 0);
      ...
      const share = itemizedTotal > 0 ? (itemAmount / itemizedTotal) * totalTaxAndTips : 0;
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-gray-900 leading-tight">Overview</h1>
          <p className="text-xs md:text-base text-gray-600 mt-1 font-medium">What's happening with your expenses.</p>
        </div>
      </header>
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 rounded-2xl md:rounded-3xl p-6 md:p-8 text-white shadow-xl shadow-indigo-200 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <p className="text-indigo-100 font-medium mb-1 text-sm">Total To Pay</p>
          <h2 className="text-3xl md:text-5xl font-bold mb-4 md:mb-6">${totals.totalUnpaid.toFixed(2)}</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        <Card className="p-4 md:p-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-gray-600 text-xs md:text-sm font-medium">Unpaid</p>
          </div>
          <p className="text-2xl md:text-3xl font-bold text-gray-900">${totals.totalUnpaid.toFixed(2)}</p>
        </Card>
        <Card className="p-4 md:p-6">
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp size={16} className="text-green-500" />
            <p className="text-gray-600 text-xs md:text-sm font-medium">Paid</p>
          </div>
          <p className="text-2xl md:text-3xl font-bold text-gray-900">${totals.totalPaid.toFixed(2)}</p>
        </Card>
        <Card className="p-4 md:p-6">
          <div className="flex items-center gap-2 mb-2">
            <p className="text-gray-600 text-xs md:text-sm font-medium">Pending</p>
          </div>
          <p className="text-2xl md:text-3xl font-bold text-gray-900">${totals.totalPending.toFixed(2)}</p>
        </Card>
        <Card className="p-4 md:p-6">
          <p className="text-gray-600 text-xs md:text-sm mb-2 font-medium">Total Bills</p>
          <p className="text-2xl md:text-3xl font-bold text-gray-900">{totalBills}</p>
        </Card>
      </div>

      {/* Bills List */}
      <div>
        <h3 className="font-bold text-base md:text-lg text-gray-900 mb-3 md:mb-4">Recent Bills</h3>
        <div className="space-y-3">
          {bills.length === 0 ? (
            <Card className="p-8 md:p-10 flex flex-col items-center justify-center text-gray-500 border-dashed">
              <Receipt size={36} className="mb-3 md:mb-4 opacity-30" />
              ...
              >
                <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
                  <div className="w-10 h-10 md:w-12 md:h-12 bg-indigo-50 rounded-xl flex-shrink-0 flex items-center justify-center text-indigo-600 group-hover:scale-110 transition-transform">
                    <Receipt size={18} className="md:block hidden" />
                    <Receipt size={16} className="md:hidden" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-gray-900 text-sm md:text-base truncate">{bill.description}</h4>
                    <p className="text-xs text-gray-500 truncate">
                      Paid by {bill.paidByName || bill.paidBy}
                    </p>
                  </div>
                </div>
