...
export const useData = (user: User | null) => {
  ...
  useEffect(() => {
    ...
    const updateBills = () => {
      ...
      uniqueBills.sort((a, b) => b.timestamp?.toMillis?.() - a.timestamp?.toMillis?.());
      setBills(uniqueBills);
    };

    const unsubscribePayerBills = onSnapshot(payerBillsQuery, (snapshot) => {
    ...
    const unsubscribeSplitBills = onSnapshot(splitBillsQuery, (snapshot) => {
